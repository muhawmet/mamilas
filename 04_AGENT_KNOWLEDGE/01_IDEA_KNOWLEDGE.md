...
A visual beat is not a paraphrase or a generic object. It is a physical arrangement and change that makes the source legible. Generate it from four questions:

1. What must the audience notice?
2. What concrete object or spatial relationship can carry that idea?
3. What single change proves it?
4. What final state makes the lesson/claim unmistakable?

Weak: "A lesson object shows comparison."
Strong: "Two differently textured shells enter matching silhouette gates; one passes, the other catches at a mismatched ridge, leaving similarity and difference visible in one frame."

## Function logic

- Hook: creates a visual question or irresistible state.
- Question: establishes uncertainty without answering.
- Reveal: exposes a hidden rule or mechanism.
- Build: assembles parts toward understanding.
- Proof: demonstrates the rule on a concrete example.
- Contrast: puts alternatives in one readable relation.
- Turn: changes interpretation or emotional direction.
- Task: creates an actionable learner/viewer challenge.
- Resolution: completes the core logic.
- Signature: leaves a memorable final image or brand/lesson mark.

Never label the final scene Build/Proof because a classifier found no keyword.

## Camera intent library

Use intent, not a preset list: enter an existing mechanism; reveal scale; move from consequence to cause; align two competing objects; let foreground occlusion disclose the answer; hold while the object proves the point; shift from child-eye uncertainty to stable teaching view. The camera may remain static when transformation is sufficient.
